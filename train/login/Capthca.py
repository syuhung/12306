...
from train.image_captcha import cut_image as ci
...
class Captcha(object):
    ...
    def verifyCodeAuto(self):
        try:
            response = EasyHttp.send(autoVerifyUrls['12306'])
    ...
    def verifyCodeAutoByMyself(self):
        try:
            ...
            address = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'image_captcha')
            ...
            with open(file_path, "wb") as f:
                ...
                f.write(imgByteArr)
                print("写入图片文件")
            results = ci.cut_image(address,file_name)
            results = self.__indexTransCaptchaResults(results)
